<template>
    <div class="h-screen grid grid-cols-8 bg-[#E9ECEF]">
        <nav class="col-span-2 bg-[#DEE2E6] px-8 py-6 flex flex-col gap-0.5 text-lg">
            ...
            <account-nav-item v-if="user && user.admin" :active="navItemActive === 'dashboard'"
                @click="navItemActive = 'dashboard'" label="Dashboard" icon="fa-solid fa-gauge"></account-nav-item>
            <account-nav-item v-if="user && user.admin" :active="navItemActive === 'users'"
                @click="navItemActive = 'users'" label="Users" icon="fa-solid fa-users"></account-nav-item>
            <account-nav-item v-if="user && user.admin" :active="navItemActive === 'products'"
                @click="navItemActive = 'products'" label="Products" icon="fa-solid fa-tags"></account-nav-item>
            <account-nav-item @click.prevent="logout" label="Logout"
                icon="fa-solid fa-right-from-bracket"></account-nav-item>
        </nav>
        <section class="col-span-6">
            <account-details v-if="navItemActive === 'details'" :user="{ ...user }"></account-details>
